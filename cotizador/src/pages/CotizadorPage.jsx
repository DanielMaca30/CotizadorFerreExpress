/**
 * CotizadorPage.jsx  v6
 * BUGS: numero en config, total alias, hasChanges indicator, Ctrl+S, safe navigate
 */
...
export default function CotizadorPage() {
  ...
  const addItem    = useCallback(()=>setItems((p)=>[...p,blankRow()]),[]);
  ...
  const PanelCotizacion = () => (
    <Stack spacing={3} p={4}>
      ...
      <Box>
        <FL>N° Cotización</FL>
        <Box px={3} py="7px" rounded="md"
          bg={useColorModeValue("gray.100","gray.700")}
          border="1px solid" borderColor={useColorModeValue("gray.200","gray.600")}>
          <Text fontSize="sm" fontWeight="700" color={cotConfig.numero?FY:useColorModeValue("gray.400","gray.500")}>
      ...
      <Flex gap={2}>
        ...
        <Box flex={1}><FL>Desc. global (%)</FL>
          <Input {...ip} type="number" min="0" max="100" value={descLocal} onChange={(e)=>setDescLocal(e.target.value)}/>
        </Box>
      </Flex>
      <Divider/>
  ...
  const PanelEmpresa = () => (
    <Box p={4}>
      ...
      <Box border="1.5px dashed" borderColor={border} rounded="lg" p={3} textAlign="center" mb={4} bg={useColorModeValue("gray.50","blackAlpha.200")}>
        ...
        <Button size="xs" variant="outline" rounded="md" leftIcon={<FiUpload size={11}/>} onClick={()=>logoRef.current?.click()}>
      ...
      <Stack spacing={3}>
        ...
        <Flex gap={2}>
          <Box flex={1}><FL>NIT</FL><Input {...ip} {...E("nit")}/></Box>
          <Box flex={1}><FL>Ciudad</FL><Input {...ip} {...E("ciudad")}/></Box>
        </Flex>
        <Box><FL>Dirección</FL><Input {...ip} {...E("dir")}/></Box>
        <Flex gap={2}>
          <Box flex={1}><FL>Teléfono</FL><Input {...ip} {...E("tel")}/></Box>
          <Box flex={1}><FL>Correo</FL><Input {...ip} {...E("correo")}/></Box>
        </Flex>
      </Stack>
    </Box>
  );
  ...
  const PanelCliente = () => (
    <Stack spacing={3} p={4}>
      ...
      <Flex gap={2}>
        <Box flex={1}><FL>Correo</FL><Input {...ip} placeholder="correo@ejemplo.com" {...C("correo")}/></Box>
    ...
    <Box minH="100vh" bg={useColorModeValue("gray.100","gray.900")}>
      ...
      <Box maxW="900px" mx="auto" py={6} px={{base:2,md:4}}>
        ...
        <Flex justify="center" mt={5} pb={8}>
          <Button size="lg" bg={DARK} color={FY} rounded="full" px={10} leftIcon={<FiDownload/>} isLoading={pdfLoading} _hover={{bg:"#2a2a28"}} onClick={handlePDF}>Descargar PDF</Button>
        </Flex>
      </Box>
    </Box>
  );
      ...
      <Box bg={barBg} borderBottom="1px solid" borderColor={border} position="sticky" top={0} zIndex={100}>
        <Flex maxW="1600px" mx="auto" px={{base:3,md:6}} h={{base:"auto",md:"54px"}} py={{base:2,md:0}} align="center" justify="space-between" flexWrap="wrap" gap={2}>
          ...
          <HStack spacing={1} flexWrap="wrap">
            <Text fontSize="12px" fontWeight="700" color={useColorModeValue("gray.700","gray.300")} display={{base:"none",lg:"block"}} mr={1}>
              {money(totals.totalPagar,cotConfig.moneda)}
            </Text>
            <Tooltip label="Historial" hasArrow>
              <IconButton size="sm" variant="outline" rounded="md" aria-label="Historial" icon={<FiList/>} onClick={()=>safeNavigate("/historial")}/>
            </Tooltip>
            {editingId&&(
              <Tooltip label="Duplicar" hasArrow>
                <IconButton size="sm" variant="outline" rounded="md" aria-label="Duplicar" icon={<FiCopy/>} onClick={handleDuplicate}/>
              </Tooltip>
            )}
            <Tooltip label="Nueva cotización" hasArrow>
              <IconButton size="sm" variant="outline" rounded="md" aria-label="Nueva" icon={<FiRefreshCw/>} onClick={handleClear}/>
            </Tooltip>
            {editingId&&(
              <Tooltip label="Eliminar" hasArrow>
                <IconButton size="sm" colorScheme="red" variant="ghost" rounded="md" aria-label="Eliminar" icon={<FiTrash2/>} onClick={onDelOpen}/>
              </Tooltip>
            )}
            <Button size="sm" variant="outline" rounded="md" leftIcon={<FiEye/>} onClick={()=>safeNavigate("preview")}>
            ...
            <Tooltip label={<HStack><Text>Guardar</Text><Kbd fontSize="10px">Ctrl+S</Kbd></HStack>} hasArrow>
              <Button size="sm" bg={hasChanges?FY:"gray.200"} color={hasChanges?DARK:"gray.500"} rounded="md" fontWeight="700"
                leftIcon={<FiSave/>} isLoading={isSaving} _hover={{bg:hasChanges?"#e0b010":"gray.300"}} onClick={handleSave}>
        ...
        <Box display={{base:"none",xl:"grid"}} gridTemplateColumns="276px 1fr 280px" gap={4} h="calc(100vh - 80px)" overflow="hidden">
            ...
            <Tabs variant="unstyled" size="sm" display="flex" flexDirection="column" h="full">
              <TabList borderBottom="1px solid" borderColor={border} px={1} pt={1} gap={0.5}>
                {[{label:"Empresa",icon:FiHome},{label:"Cliente",icon:FiUser},{label:"Cotización",icon:FiFileText}].map(({label,icon:Ic})=>(
                  <Tab key={label} flex={1} fontSize="9px" fontWeight="700" letterSpacing="0.1em" textTransform="uppercase" color={mutedL} pb={2.5}
          ...
          <Box display="flex" flexDirection="column" gap={3} minW={0}>
            ...
            <GlassCard rounded="xl" flex={1} overflow="hidden" display="flex" flexDirection="column" minH={0}>
              <Box overflowX="auto" overflowY="auto" flex={1} sx={{"&::-webkit-scrollbar":{w:"4px",h:"4px"},"&::-webkit-scrollbar-thumb":{bg:"gray.200",borderRadius:"2px"}}}>
                  ...
                  <Thead position="sticky" top={0} zIndex={1}>
                    <Tr bg={theadBg}>
                      {[{h:"#",w:"28px"},{h:"Ref.",w:"60px"},{h:"Descripción"},{h:"Cant.",w:"52px",num:true},{h:"Und.",w:"54px"},
                        ...
                        .map(({h,w,num},i)=>(
                          <Th key={i} color={FY} borderColor="transparent" fontSize="8px" letterSpacing="wider" fontWeight="700" isNumeric={num} w={w}>{h}</Th>
                        ))}
                    </Tr>
                  </Thead>
                  <Tbody>
                      ...
                      {items.map((r,i)=>{
                        ...
                        return (
                          <MotionTr key={r.id} layout
                            ...
                            <Td borderColor={border} p={1}>
                              <Input variant="unstyled" value={r.desc} onChange={(e)=>upItem(r.id,"desc",e.target.value)}
                                placeholder="Nombre del producto…" fontSize="12px" fontWeight="600" px={2} py={1} rounded="md"
                                _hover={{bg:inputBg}} _focus={{bg:inputBg,boxShadow:`0 0 0 1.5px ${FY}55`}}/>
                            </Td>
                            ...
                            <Td borderColor={border} p={1}>
                              <Tooltip label="Eliminar fila" hasArrow>
                                <IconButton size="xs" variant="ghost" colorScheme="red" rounded="md" aria-label="Eliminar"
                    ...
                    <Tr>
                      <Td colSpan={11} borderColor={border} px={3} py={2}>
                        <Button size="xs" variant="ghost" leftIcon={<FiPlus/>} onClick={addItem} fontWeight="700" color={FY}
          ...
          <Box bg={DARK} rounded="xl" overflow="hidden" display="flex" flexDirection="column" border="1px solid" borderColor="whiteAlpha.100">
            ...
            <Box px={5} py={4}>
              ...
              <Button w="full" bg={hasChanges?"whiteAlpha.200":"whiteAlpha.100"} color="white" rounded="lg" leftIcon={<FiSave/>} isLoading={isSaving} onClick={handleSave} _hover={{bg:"whiteAlpha.300"}}>
                {editingId?"Actualizar":"Guardar"}
              </Button>
            </Box>
          </Box>
                ...
                <Tbody>
                  {items.map((r,i)=>(
                    <Tr key={r.id} bg={i%2===0?tableBg:stripeBg}>
                      <Td borderColor={border} color={mutedL} fontSize="11px" pl={3}>{i+1}</Td>
                      <Td borderColor={border} p={1}><Input variant="unstyled" value={r.desc} onChange={(e)=>upItem(r.id,"desc",e.target.value)} placeholder="Producto…" fontSize="12px" fontWeight="600" px={2} py={1} rounded="md" _focus={{bg:inputBg,boxShadow:`0 0 0 1.5px ${FY}55`}}/></Td>
                      <Td borderColor={border} p={1} isNumeric><Input variant="unstyled" type="number" value={r.qty} onChange={(e)=>upItem(r.id,"qty",e.target.value)} textAlign="center" fontWeight="700" fontSize="12px" px={1} py={1} rounded="md" w="44px" _focus={{bg:inputBg,boxShadow:`0 0 0 1.5px ${FY}55`}}/></Td>
                      <Td borderColor={border} p={1} isNumeric><Input variant="unstyled" type="number" value={r.price} onChange={(e)=>upItem(r.id,"price",e.target.value)} placeholder="0" textAlign="right" fontSize="12px" fontWeight="600" px={2} py={1} rounded="md" w="88px" _focus={{bg:inputBg,boxShadow:`0 0 0 1.5px ${FY}55`}}/></Td>
                      <Td borderColor={border} isNumeric pr={2}><Text fontWeight="700" fontSize="12px">{money(calcRow(r),cotConfig.moneda)}</Text></Td>
                      <Td borderColor={border} p={1}><IconButton size="xs" variant="ghost" colorScheme="red" rounded="md" aria-label="Eliminar" icon={<FiTrash2 size={12}/>} onClick={()=>removeItem(r.id)} isDisabled={items.length===1}/></Td>
                    </Tr>
                  ))}
                  <Tr><Td colSpan={6} borderColor={border} px={3} py={2}><Button size="xs" variant="ghost" leftIcon={<FiPlus/>} onClick={addItem} fontWeight="700" color={FY} _hover={{bg:useColorModeValue("yellow.50","whiteAlpha.100")}}>+ Agregar producto</Button></Td></Tr>
                </Tbody>
              </Table>
            </Box>
          </GlassCard>
      ...
      <AlertDialog isOpen={isDelOpen} leastDestructiveRef={cancelRef} onClose={onDelClose}>
        <AlertDialogOverlay><AlertDialogContent rounded="xl">
          ...
          <AlertDialogFooter>
            ...
            <Button bg={RED} color="white" ml={3} rounded="md" onClick={()=>{onDelClose();handleDelete();}}>Eliminar</Button>
          </AlertDialogFooter>
